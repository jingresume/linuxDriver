...
static int led_pdev_probe(struct platform_device* pdev)
{
    ...
    if (!cur_led) return -ENOMEM;

    led_hwinfo = devm_kzalloc(&pdev->dev, sizeof(uint) * 2, GFP_KERNEL);
    if (!led_hwinfo) return -ENOMEM;
...
static int led_pdev_remove(struct platform_device* pdev)
{
    dev_t cur_dev;
...
static struct platform_driver led_pdrv = {
    .probe       = led_pdev_probe,
    .remove      = led_pdev_remove,
